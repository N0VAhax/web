define([
    ...
    function Engine() {
        ...
        // self.chatws = io(AppConstants.Engine.CHAT_HOST);

        self.isChatWsConnected = false;
        ...
        self.tickTimer = null;
        self.calcBonusesIntervalID = null;
        ...
        self.ws.on('game_started', function(bets) {
            ...
            Object.keys(bets).forEach(function(username) {
                ...
                self.playerInfo[username] = { bet: bets[username].bet, autoCashOut: bets[username].autoCashOut };
            });

            self.calcBonuses();
            if (self.calcBonusesIntervalID) {
                clearInterval(self.calcBonusesIntervalID);
            }
            window.calcBonusesIntervalID = setInterval(function(){
                self.calcBonuses();
            }, 300);
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            console.log("cashed_out", self.playerInfo[resp.username]);
        ...
        self.ws.on('connect', function() {
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: ott, api_version: 1 },
                    function(err, resp) {
                        if (err) {
    ...
    Engine.prototype.say = function(msg) {
        ...
        this.ws.emit('say', msg, false, 'generalChat');
    };
