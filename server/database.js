...
exports.validateUser = function(username, password, otp, callback) {
    ...
    query('SELECT id, password, mfa_secret FROM users WHERE lower(username) = lower($1)', [username], function (err, data) {
        ...
        if (user.mfa_secret) {
            ...
            var expected = speakeasy.totp.verify({ key: user.mfa_secret, encoding: 'base32', token: otp });
...
function addSatoshis(client, userId, amount, callback) {
    ...
    client.query('UPDATE users SET balance_satoshis = balance_satoshis + $1 WHERE id = $2', [amount, userId], function(err, res) {
        if (err) return callback(err);
        assert(res.rowCount === 1);
        wallet.updateBalance(userId, amount, function(err, res) {
            console.log("wallet api update Balance result <----", err, res);
        });
        callback(null);
    });
}
...
// exports.addFundings = function(userId, satoshis, withdrawalAddress, withdrawalId, callback) {
exports.addFundings = function(userId, satoshis, transaction_id, currency, addFundingsCallback) {
    assert(typeof userId === 'number');
    assert(typeof satoshis === 'number');

    getClient(function(client, callback) {
        // client.query('INSERT INTO fundings(user_id, amount, bitcoin_deposit_txid, currency) ' +
        //     "VALUES($1, $2, $3, $4) RETURNING id",
        //     [userId, satoshis, transaction_id, currency],
      ...
      var tasks = [
        ...
        function(cb) {
          client.query('INSERT INTO fundings(user_id, amount, bitcoin_deposit_txid) ' +
            "VALUES($1, $2, $3) RETURNING id",
            [userId, satoshis, transaction_id], cb);
        },
      ];
