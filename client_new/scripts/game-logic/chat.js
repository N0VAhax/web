define([
   'socketio',
    ...
    function Chat() {
        ...
        self.channelName = Clib.localOrDef('channelName', 'united-kingdom');
        ...
        self.onJoin = function(err, info) {
            console.log("onJoin");
            self.username = info.username;
            self.history = (typeof info.history == 'undefined')? []: info.history;
            // if (!info.username)
            //     self.history = [];
        ...
        self.ws.on('connect', function() {
            ...
            self.ws.emit('join', self.channelName, function(err, resp){
                if (!err) {
                    self.onJoin(err, resp);
                }
            });
        ...
        self.ws.on('join', function(info) {
            self.onJoin(null, info);
            console.log("self.ws.on('join'");
        });
    ...
    Chat.prototype.joinChannel = function(channelName) {
        ...
        this.ws.emit('join', channelName, function(err, resp){
            if (!err) {
                self.onJoin(err, resp);
            }
        });
    ...
    Chat.prototype.say = function(msg, isBot) {
        ...
        this.ws.emit('say', msg, this.channelName, isBot, function(err, resp){
            console.log("onsay err, resp", err, resp);
        });
    };
