define([
    ...
    function Engine() {
        ...
        /** Leaderboard Info **/
        self.getLeaderBoard = function(sort, callback) {
            requestLeaderBoardInfo(sort, function(err, data) {
                if (err & err != 401) {
                    console.error('request leaderboardInfo error:', err);
                    callback(err);
                } else {
                    var res = JSON.parse(data);
                    callback(null, res.leaders);
                }
            });
        }

        /** Security Info **/
        self.requestSecurityInfo = function(callback) {
            requestSecurityInfo(function(err, data) {
                if (err & err != 401) {
                    console.error('request leaderboardInfo error:', err);
                    callback(err);
                } else {
        ...
        self.changePassword = function(data, callback) {
            requestChangePassword(data, function(err, res) {
                console.log(err);
                if (err && err != 401) {
                    console.error('request changePassword error:', err);
                    callback(err);
                } else {
                    callback(null, res);
                }
        ...
        /** Change Email **/
        self.changeEmail = function(data, callback) {
            requestChangeEmail(data, function(err, res) {
                console.log(err);
                if (err && err != 401) {
                    console.error('request changeEmail error:', err);
                    callback(err);
                } else {
                    callback(null, res);
                }
        ...
        /** Change mfa status **/
        self.changeMfaStatus = function(data, callback) {
            requestChangeMfa(data, function(err, res) {
                console.log(err);
                if (err && err != 401) {
                    console.error('request changeMfa error:', err);
                    callback(err);
                } else {
                    callback(null, res);
                }
        ...
        self.logOut = function(callback) {
            requestLogout(function(err, res) {
                console.log(err);
                if (err && err != 401) {
                    console.error('request logout error:', err);
                    callback(err);
                } else {
                    callback(null, res);
                }
        ...
        self.requestContact = function(data, callback) {
            requestContact(data, function(err, res) {
                console.log(err);
                if (err && err != 401) {
        ...
        self.ws.on('game_started', function(bets) {
            ...
            Object.keys(bets).forEach(function(username) {
                if (self.username === username)
                    self.balanceSatoshis -= bets[username].bet;

                self.playerInfo[username] = { bet: bets[username].bet, username: username, autoCashOut: bets[username].autoCashOut };
            });
        ...
        self.ws.on('game_crash', function(data) {
            ...
            self.nyan = false;
            console.log(self.balanceSatoshis)
            requestUpdateWallet(self.balanceSatoshis, function(err, res) {
                if (err) {
                    alert("while updating wallet, occured error!");
                }
        ...
        self.ws.on('cashed_out', function(resp) {
            ...
            console.log("cashed_out", resp, self.playerInfo[resp.username]);
            console.log("cashed_outxx", self.playerInfo[resp.username].bet * resp.stopped_at / 100);
        ...
        self.ws.on('connect', function() {
            ...
            requestOtt(function(err, ott) {
                ...
                self.ws.emit('join', { ott: window.DEV_OTT? window.DEV_OTT : ott, api_version: 1 },
                    function(err, resp) {
                        if (err) {
    ...
    Engine.prototype.calcBonuses = function() {
        var self = this;
        var crashPoint = Clib.calcGamePayout(Clib.getElapsedTimeWithLag(self))*100;
        ...
        slideSameStoppedAt(playersArrSorted,
            function(array, listOfRecordsPositions, cashOutAmount, totalBetAmount) {
                ...
                for (var i = 0, length = listOfRecordsPositions.length; i < length; i++) {
                    ...
                    var rpi = listOfRecordsPositions[i];
                    var toAlloc = Math.min(bonusPool, maxWinRatio*array[rpi].bet);//var toAlloc = (array[rpi].bet / totalBetAmount) * toAllocAll;
                    if (array[rpi].autoCashOut > crashPoint) {
                        toAlloc = 0;
                    }
                    bonusPool -= toAlloc;

                    array[rpi].bonus = toAlloc;
                }
                self.trigger('bonus_change');
            }
        );
    }
    ...
    function requestLeaderBoardInfo(sort, callback) {
        try {
            var ajaxReq = new XMLHttpRequest();
            var url = "/leaderboardInfo";
            if (!ajaxReq)
                throw new Error("Your browser doesn't support xhr");
    ...
    function requestSecurityInfo(callback) {
        try {
            var ajaxReq = new XMLHttpRequest();
            var url = "/newSecurity";
            if (!ajaxReq)
    ...
    function requestChangePassword(data, callback) {
        try {
            var ajaxReq  = new XMLHttpRequest();
            ...
            ajaxReq.open("POST", "/new-reset-password", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            var body = {'old_password': data.old_password, 'password': data.password, 'confirmation': data.confirmation, 'otp': data.otp};
            ajaxReq.send(JSON.stringify(body));
    ...
    function requestChangeEmail(data, callback) {
        try {
            var ajaxReq  = new XMLHttpRequest();
            ...
            ajaxReq.open("POST", "/new-edit-email", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            var body = {'password': data.password, 'email': data.email, 'otp': data.otp};
            ajaxReq.send(JSON.stringify(body));
    ...
    function requestChangeMfa(data, callback) {
        try {
            var ajaxReq  = new XMLHttpRequest();
            ...
            if (data.status)
                ajaxReq.open("POST", "/disable-mfa", true);
            else 
                ajaxReq.open("POST", "/enable-mfa", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            if (data.status)
                var body = {'otp': data.otp};
            else 
                var body = {'otp': data.otp, 'mfa_potential_secret': data.mfa_potential_secret, 'sig': data.sig};
            ajaxReq.send(JSON.stringify(body));
    ...
    function requestLogout(callback) {
        try {
            ...
            ajaxReq.open("POST", "/newLogout", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            ajaxReq.send();
    ...
    function requestContact(data, callback) {
        try {
            ...
            ajaxReq.open("POST", "/newContact", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            var body = {
                'email': data.email,
    ...
    function requestUpdateWallet(data, callback) {
        try {
            var ajaxReq  = new XMLHttpRequest();
            ...
            ajaxReq.open("POST", "/updateWallet", true);
            ajaxReq.setRequestHeader('Accept', 'application/json');
            ajaxReq.setRequestHeader('Content-Type', 'application/json');
            var body = {
                'amount': data
