define([
    ...
    return {
        ...
        payout: function(betSize, ms) {
            return betSize * Math.pow(Math.E, (0.00008*ms));
        },

        payoutTime: function (betSize, payout) {
            return Math.log(payout/betSize)/0.00008;
        },
        ...
        parseBet: function (betString) {
            ...
            if (!/^\d+k*$/.test(betString))
                return new Error('Bet may only contain digits, or k (to mean 1000)');

            var bet = parseInt(betString.replace(/k/g, '000'));

            if (bet < 1)
                return new Error('The bet should be at least one');

            if (bet * 1e6 > AppConstants.Engine.MAX_BET)
                return new Error('The bet must be less than 50,000 mETH');
        ...
        calcGamePayout: function(ms) {
            ...
            var gamePayout = this.growthFuncNoFloor(ms);
